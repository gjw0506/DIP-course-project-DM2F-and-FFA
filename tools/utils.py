...
def sliding_forward(net, x: torch.Tensor, crop_size=240):#512):
    n, c, h, w = x.size()
...
def sliding_forward_for_hazerd(net, x: torch.Tensor, crop_size=(460,620)):#1536):
    n, c, h, w = x.size()
    ...
    if h <= crop_size[0] and w <= crop_size[1]:
        ...
        result = torch.zeros(n, c, h, w).cuda()
        count = torch.zeros(n, 1, h, w).cuda()
        stride_h = int(crop_size[0] / 3.)
        stride_w = int(crop_size[1] / 3.)


        h_steps = 1 + int(math.ceil(float(max(h - crop_size[0], 0)) / stride_h))
        w_steps = 1 + int(math.ceil(float(max(w - crop_size[1], 0)) / stride_w))

        for h_idx in range(h_steps):
            for w_idx in range(w_steps):
                ws0, ws1 = w_idx * stride_w, crop_size[0] + w_idx * stride_w
                hs0, hs1 = h_idx * stride_h, crop_size[1] + h_idx * stride_h
                if h_idx == h_steps - 1:
